...
function rotateLabels() {
  ...
  d3.event.preventDefault();
    ...
    function wrap(labels, width) {
        labels.each(function() {
            //this has to be an svg element to have .text() and .attr()
            console.log(d3.select(this)[0][0].__data__);
            var text = d3.select(this)[0][0],
                words = text.textContent.split(/\s+/).reverse(),
            //words = text.__data__.split(/\s+/).reverse(),
                word,
                line = [],
                lineNumber = 0,
                lineHeight = 1.1, // ems
                y = text.getAttribute("y") // text.attr("y"),
                dy = parseFloat(text.getAttribute("dy")),
  ...
  //d3.selectAll(".label").call(wrap, 50);
